const model = require('../models/PolicePermitModel');

class PolicePermitDao {
    ...
    static create(details) {
        return new Promise((resolve, reject) => {
            (new model(details)).save()
                .then((result) => {
                    const police_no = new Date().getTime().toString() + result.auto_id.toString();
                    return this.modifyById(result._id, { police_no })
                })
                .then((result) => {
                    resolve(result)
                })
...
module.exports = PolicePermitDao;