const model = require('../models/BarangayPermitModel');

class BarangayPermitDao {
    ...
    static create(details) {
        return new Promise((resolve, reject) => {
            (new model(details)).save()
                .then((result) => {
                    const barangay_no = new Date().getTime().toString() + result.auto_id.toString();
                    return this.modifyById(result._id, { barangay_no })
                })
                .then((result) => {
                    resolve(result)
                })
...
module.exports = BarangayPermitDao;